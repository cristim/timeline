...
func TestGoldenGateBlocksPublish(t *testing.T) {
	...
	m, _, err := Run(context.Background(), sink, nil, "test", "seed-x", es, testBasemap(), &model.GeoSet{}, bad)
	if err == nil || m != nil {
	...
	m, _, err = Run(context.Background(), sink, nil, "test", "seed-x", es, testBasemap(), &model.GeoSet{}, good)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGoldenSeedVersionPin(t *testing.T) {
	...
	_, _, err := Run(context.Background(), newMemSink(), nil, "test", "seed-x", es, testBasemap(), &model.GeoSet{}, stale)
